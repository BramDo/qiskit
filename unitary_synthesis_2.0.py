...
sis_plugin_names`
        ...
        # Bypass target if it doesn't contain any basis gates (i.e it's _FakeTar
get), as this
        # not part of the official target model.
        self._target = target if target is not None and len(target.operation_nam
es) > 0 else None
...
n plugin list")
 ...
 the iteration
        ...
        if self.plugins:
            ...
            from qiskit.transpiler.passes.synthesis.default_unitary_synth_plugin
 import (
 ...
 None else set()
            ...
            for method, kwargs in method_list:
                ...
                if method.supports_gate_lengths:
                    _gate_lengths = _gate_lengths or _build_gate_lengths(self._t
arget)
                    kwargs["gate_lengths"] = _gate_lengths
                if method.supports_gate_errors:
                    _gate_errors = _gate_errors or _build_gate_errors(self._targ
et)
...
en completed."""
        ...
        for node in dag.topological_op_nodes():
            if node.name in self._synth_gates and len(node.qargs) >= self._min_q
ubits:
                synth_dag = None
                unitary = node.matrix
