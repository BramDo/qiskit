...
================================================================================
=========
Unitary Synthesis Plugin (in :mod:`qiskit.transpiler.passes.synthesis.unitary_sy
nthesis`)
================================================================================
=========
...
def _decomposer_2q_from_basis_gates(basis_gates, pulse_optimize=None, approximat
ion_degree=None):
    decomposer2q = None
    ...
    if isinstance(kak_gate, RZXGate):
        ...
        decomposer2q = XXDecomposer(euler_basis=euler_basis, backup_optimizer=ba
ckup_optimizer)
        ...
        try:
            ...
            for key in keys:
                ...
                if isinstance(target_op, op.base_class) and (
                    ...
                    or all(
                        isclose(float(p1), float(p2)) for p1, p2 in zip(target_o
p.params, op.params)
                    )
                ):
            ...
            raise TranspilerError(
                f"Encountered a bad synthesis. " f"Target has no {op} on qubits
{qubits}."
            ) from error

    if isinstance(circuit, DAGCircuit):
        for inst in circuit.topological_op_nodes():
            inst_qubits = tuple(qubits[circuit.find_bit(q).index] for q in inst.
qargs)
...
def _preferred_direction(
    decomposer2q, qubits, natural_direction, coupling_map=None, gate_lengths=Non
e, gate_errors=None
):
    """
    `decomposer2q` decomposes an SU(4) over `qubits`. A user sets `natural_direc
tion`
    to indicate whether they prefer synthesis in a hardware-native direction.
    If yes, we return the `preferred_direction` here. If no hardware direction i
s
...
class UnitarySynthesis(TransformationPass):
    ...
    @deprecate_arg(
        ...
        additional_msg="The BackendProperties data structure has been deprecated
 and will be "
        "removed in Qiskit 2.0. The `target` input argument should be used inste
ad. "
        "You can use Target.from_configuration() to build the target from the pr
operties "
    ...
    def __init__(
        ...
        Args:
            ...
            approximation_degree (float): heuristic dial used for circuit approx
imation
                (1.0=no approximation, 0.0=maximal approximation). Approximation
 can
                make the synthesized circuit cheaper at the cost of straying fro
m
                the original unitary. If None, approximation is done based on ga
te fidelities.
            coupling_map (CouplingMap): the coupling map of the backend
                in case synthesis is done on a physical circuit. The
                directionality of the coupling_map will be taken into
                account if ``pulse_optimize`` is ``True``/``None`` and ``natural
_direction``
                is ``True``/``None``.
            ...
            natural_direction (bool): Whether to apply synthesis considering
                ...
                ``pulse_optimize`` is ``True`` or ``None``. The natural directio
n is
            ...
            min_qubits: The minimum number of qubits in the unitary to synthesiz
e. If this is set
                and the unitary is less than the specified number of qubits it w
ill not be
                synthesized.
            ...
            target: The optional :class:`~.Target` for the target device the pas
s
...
.
        ...
        Raises:
            TranspilerError: if ``method`` was specified but is not found in the
                installed plugins list. The list of installed plugins can be que
ried with
                :func:`~qiskit.transpiler.passes.synthesis.plugin.unitary_synthe
sis_plugin_names`
        """
        super().__init__()
        ...
        if self.method != "default" and self.method not in self.plugins.ext_plug
ins:
            raise TranspilerError(f"Specified method '{self.method}' not found i
n plugin list")
    ...
    def run(self, dag: DAGCircuit) -> DAGCircuit:
        ...
        # If there aren't any gates to synthesize in the circuit we can skip all
 the iteration
        # and just return.
        ...
        if self.method == "default":
            # If the method is the default, we only need to evaluate one set of
keyword arguments.
            # To simplify later logic, and avoid cases where static analysis mig
ht complain that we
            # haven't initialized the "default" handler, we rebind the names so
they point to the
            ...
            # If the method is not the default, we still need to initialise the
default plugin's
            # keyword arguments in case we have to fall back on it during the ac
tual run.
            default_method = self.plugins.ext_plugins["default"].obj
            default_kwargs = {}
            method_list = [(plugin_method, plugin_kwargs), (default_method, defa
ult_kwargs)]

        # Handle approximation degree as a special case for backwards compatibil
ity, it's
        # not part of the plugin interface and only something needed for the def
ault
        # pass.
        ...
        qubit_indices = (
            ...
            if plugin_method.supports_coupling_map or default_method.supports_co
upling_map
            else {}
        )
        ...
        if self.method == "default" and self._target is not None:
            _coupling_edges = (
                set(self._coupling_map.get_edges()) if self._coupling_map is not
 None else set()
            )
            ...
            for method, kwargs in method_list:
                ...
                if method.supports_gate_lengths_by_qubit:
                    _gate_lengths_by_qubit = _gate_lengths_by_qubit or _build_ga
te_lengths_by_qubit(
                        self._backend_props, self._target
                    )
                    kwargs["gate_lengths_by_qubit"] = _gate_lengths_by_qubit
                if method.supports_gate_errors_by_qubit:
                    _gate_errors_by_qubit = _gate_errors_by_qubit or _build_gate
_errors_by_qubit(
                        self._backend_props, self._target
                    )
                ...
                if supported_bases is not None:
                    kwargs["matched_basis"] = _choose_bases(self._basis_gates, s
upported_bases)
                if method.supports_target:
                    kwargs["target"] = self._target

            out = self._run_main_loop(
                dag, qubit_indices, plugin_method, plugin_kwargs, default_method
, default_kwargs
            )
            return out

    def _run_main_loop(
        self, dag, qubit_indices, plugin_method, plugin_kwargs, default_method,
default_kwargs
    ):
        """Inner loop for the optimizer, after all DAG-independent set-up has be
en completed."""
        ...
        for node in dag.topological_op_nodes():
            if node.name == "unitary" and len(node.qargs) >= self._min_qubits:
                ...
                if (
                    plugin_method.max_qubits is not None and n_qubits > plugin_m
ethod.max_qubits
                ) or (plugin_method.min_qubits is not None and n_qubits < plugin
_method.min_qubits):
...
(
                            ...
                            node = DAGOpNode.from_instruction(
                                CircuitInstruction.from_standard(
                                    gate, tuple(qubits[x] for x in qargs), param
s
                                )
                            )
                        out_dag._apply_op_node_back(node)
    ...
    if target is not None and target.qargs is not None:
        for qubits in target.qargs:
            ...
            for name in names:
                ...
                duration = getattr(target[name].get(qubits, None), "duration", N
one)
                if duration:
...
class DefaultUnitarySynthesis(plugin.UnitarySynthesisPlugin):
    ...
    def _decomposer_2q_from_target(self, target, qubits, approximation_degree):
        ...
        # available instructions on this qubit pair, and their associated proper
ty.
        available_2q_basis = {}
        available_2q_props = {}

        # 2q gates sent to 2q decomposers must not have any symbolic parameters.
  The
        # gates must be convertable to a numeric matrix. If a basis gate support
s an arbitrary
...
:
        ...
        if not available_2q_basis:
            raise TranspilerError(
                f"Target has no gates available on qubits {qubits} to synthesize
 over."
            )
        ...
        for basis_1q, basis_2q in product(available_1q_basis, supercontrolled_ba
sis.keys()):
        ...
        # If our 2q basis gates are a subset of cx, ecr, or cz then we know TwoQ
ubitBasisDecomposer
        # is an ideal decomposition and there is no need to bother calculating t
he XX embodiments
        ...
        controlled_basis = {k: v for k, v in available_2q_basis.items() if is_co
ntrolled(v)}
        basis_2q_fidelity = {}
        embodiments = {}
        pi2_basis = None
        for k, v in controlled_basis.items():
            strength = 2 * TwoQubitWeylDecomposition(Operator(v).data).a  # pi/2
: fully entangling
            ...
            # basis equivalent to CX are well optimized so use for the pi/2 angl
e if available
            if isclose(strength, pi / 2) and k in supercontrolled_basis:
                pi2_basis = v
        # if we are using the approximation_degree knob, use it to scale already
-given fidelities
        if approximation_degree is not None:
            basis_2q_fidelity = {k: v * approximation_degree for k, v in basis_2
q_fidelity.items()}
        if basis_2q_fidelity:
            for basis_1q in available_1q_basis:
                if isinstance(pi2_basis, CXGate) and basis_1q == "ZSX":
                    ...
                    embodiments.update({pi / 2: XXEmbodiments[pi2_decomposer.gat
e.base_class]})
                else:
                    pi2_decomposer = None
...
[]
            ...
            synth_circuits = []
            # If we have a single TwoQubitBasisDecomposer skip dag creation as w
e don't need to
            # store and can instead manually create the synthesized gates direct
ly in the output dag
            if len(decomposers2q) == 1 and isinstance(decomposers2q[0], TwoQubit
BasisDecomposer):
                ...
                return self._synth_su4_no_dag(
                    unitary, decomposers2q[0], preferred_direction, approximatio
n_degree
                )
            for decomposer2q in decomposers2q:
                preferred_direction = _preferred_direction(
                    decomposer2q, qubits, natural_direction, coupling_map, gate_
lengths, gate_errors
                )
                synth_circuit = self._synth_su4(
                    unitary, decomposer2q, preferred_direction, approximation_de
gree
                )
            ...
            from qiskit.synthesis.unitary.qsd import (  # pylint: disable=cyclic
-import
                qs_decomposition,
            )

            # only decompose if needed. TODO: handle basis better
            synth_circuit = qs_decomposition(unitary) if (basis_gates or target)
 else None
    ...
    def _synth_su4_no_dag(self, unitary, decomposer2q, preferred_direction, appr
oximation_degree):
        approximate = not approximation_degree == 1.0
        synth_circ = decomposer2q._inner_decomposer(unitary, approximate=approxi
mate)
        if not preferred_direction:
            return (synth_circ, synth_circ.global_phase, decomposer2q.gate)

        synth_direction = None
        # if the gates in synthesis are in the opposite direction of the preferr
ed direction
        # resynthesize a new operator which is the original conjugated by swaps.
        # this new operator is doubly mirrored from the original and is locally
equivalent.
        for gate, _params, qubits in synth_circ:
            if gate is None or gate == CXGate._standard_gate:
                synth_direction = qubits
        if synth_direction is not None and synth_direction != preferred_directio
n:
            # TODO: Avoid using a dag to correct the synthesis direction
            return self._reversed_synth_su4(unitary, decomposer2q, approximation
_degree)
        return (synth_circ, synth_circ.global_phase, decomposer2q.gate)

    def _synth_su4(self, su4_mat, decomposer2q, preferred_direction, approximati
on_degree):
        approximate = not approximation_degree == 1.0
...
equivalent.
        for inst in synth_circ.topological_op_nodes():
            if inst.op.num_qubits == 2:
                synth_direction = [synth_circ.find_bit(q).index for q in inst.qa
rgs]
        if synth_direction is not None and synth_direction != preferred_directio
n:
            return self._reversed_synth_su4(su4_mat, decomposer2q, approximation
_degree)
        return synth_circ
    ...
    def _reversed_synth_su4(self, su4_mat, decomposer2q, approximation_degree):
        ...
        su4_mat_mm[:, [1, 2]] = su4_mat_mm[:, [2, 1]]
        synth_circ = decomposer2q(su4_mat_mm, approximate=approximate, use_dag=T
rue)
        out_dag = DAGCircuit()
        ...
        for node in synth_circ.topological_op_nodes():
            qubits = tuple(flip_bits[synth_circ.find_bit(x).index] for x in node
.qargs)
            node = DAGOpNode.from_instruction(
                node._to_circuit_instruction().replace(qubits=qubits, params=nod
e.params)
            )
            out_dag._apply_op_node_back(node)
        return out_dag
